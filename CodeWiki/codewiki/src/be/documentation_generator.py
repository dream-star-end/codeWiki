...
class DocumentationGenerator:
    ...
    def _report_progress(self, current: float, total: float, detail: str) -> None:
        if not self.progress_callback:
            return
        try:
            self.progress_callback(current, total, detail)
        except Exception:
            logger.debug("progress_callback failed during documentation generation")
    ...
    async def generate_module_documentation(self, components: Dict[str, Any], leaf_nodes: List[str]) -> str:
        ...
        if len(module_tree) > 0:
            total_units = len(processing_order) + 1
            processed_count = 0
            for module_path, module_name in processing_order:
                try:
                    ...
                    if module_key in processed_modules:
                        processed_count += 1
                        self._report_progress(processed_count, total_units, module_key)
                        continue
                    
                    # Process the module
                    module_index = processed_count

                    def _sub_progress(sub_current: int, sub_total: int, sub_name: str):
                        if sub_total <= 0:
                            return
                        fraction = min(1.0, sub_current / sub_total)
                        detail = f"{module_key}/{sub_name}" if sub_name else module_key
                        self._report_progress(module_index + fraction, total_units, detail)
            ...
            processed_count += 1
            self._report_progress(processed_count, total_units, "仓库概览")
        else:
            ...
            def _sub_progress(sub_current: int, sub_total: int, sub_name: str):
                if sub_total <= 0:
                    return
                fraction = min(1.0, sub_current / sub_total)
                detail = f"{repo_name}/{sub_name}" if sub_name else repo_name
                self._report_progress(fraction, 1.0, detail)
            ...
            self._report_progress(1, 1, repo_name)
